...
{-| An Practitioner's profile.
-}

import Api exposing (Cred)
import Api.Endpoint as Endpoint
import Author exposing (Author(..), FollowedAuthor, UnfollowedAuthor)
import FHIR.Resource exposing (Resource, Preview)
import FHIR.Resources.Feed as Feed
...
fetchFeed session feedTabs username page =
    let
        ...
        params =
            firstParam :: PaginatedList.params { page = page, resultsPerPage = resourcesPerPage }

        expect =
            Feed.decoder maybeCred resourcesPerPage
    in
    Api.get (Endpoint.resources params) maybeCred expect
        |> Http.toTask
                                    ...
                                    [ div [ class "col-xs-12 col-md-10 offset-md-1" ]
                                        [ div [ class "articles-toggle" ] <|
                                            List.concat
                                                ...
                                                , Feed.viewResources model.timeZone feed
...
viewTabs tab =
    case tab of
        MyResources ->
            Feed.viewTabs [] myResources [ favoritedResources ]

        FavoritedResources ->
            Feed.viewTabs [ myResources ] favoritedResources []


myResources : ( String, Msg )
myResources =
    ( "My Resources", ClickedTab MyResources )


favoritedResources : ( String, Msg )
favoritedResources =
    ( "Favorited Resources", ClickedTab FavoritedResources )
